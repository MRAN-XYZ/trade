...
# ===================================================
# *** CHANGED: Now creates two types of labels (regression and classification) ***
def prepare_data(df, bot: DerivTradingBot):
    ...
    try:
        ...
        # Label 1: Regression target (for base models)
        # Predict the percentage return of the next candle
        y_reg = df["close"].pct_change().shift(-1).fillna(0)
        
        # Label 2: Classification target (for meta-model)
        # Thresholds for BUY (0), SELL (1), HOLD (2)
        threshold = 0.001 # 0.1% move
        y_class = np.where(y_reg > threshold, 0,  # BUY
                   np.where(y_reg < -threshold, 1, 2))  # SELL / HOLD
        
        # Ensure alignment after dropping NaNs that might arise from feature creation
...
# ===================================================
# *** CHANGED: Trains a Regressor, not a Classifier ***
def train_rf_regressor(X, y_reg):
    ...
    try:
        ...
        score = rf.score(X_test, y_test) # R-squared score
...
def train_lstm_regressor(X, y_reg, model_name, lookback=20):
    ...
    try:
        ...
        model = Sequential([
            ...
            LSTM(32),
            Dropout(0.2),
            Dense(1) # *** CHANGED: Output is a single continuous value ***
        ])
        
        # *** CHANGED: Using regression loss function ***
...
# ===================================================
# *** CHANGED: Completely refactored to match the bot's prediction logic ***
def train_meta_model(rf_model, lstm15_model, lstm60_model, X15, X60, y15_class):
    ...
    try:
        # --- Generate predictions from base models to use as features ---
        
        # RF predictions (using 15min data)
        pred_rf = rf_model.predict(X15)
        
        # LSTM 15min predictions (helper function for sequence generation)
        ...
        pred_lstm15 = get_lstm_preds(lstm15_model, scaled_X15, lookback=15) # Match lookback
        pred_lstm60 = get_lstm_preds(lstm60_model, scaled_X60, lookback=60) # Match lookback

        # --- Align all predictions and the target variable ---
        # The number of predictions will be different due to different lookbacks and data lengths.
        # We align them based on the shortest prediction array.
        min_len = min(len(pred_rf), len(pred_lstm15), len(pred_lstm60))
        
        # Important: Align data from the END of the arrays
        ...
        # --- Train the meta-model classifier ---
...
async def main():
    """Main training pipeline"""
    try:
        # Instantiate the bot to use its helper functions
        bot = DerivTradingBot(APP_ID, TOKEN, MODEL_DIR)

        # 1. Fetch data
        df15 = await fetch_candles(granularity=900, count=5000) # 15-min
        df60 = await fetch_candles(granularity=3600, count=5000) # 60-min

        # 2. Prepare features and labels for both timeframes
        X15, y15_reg, y15_class = prepare_data(df15, bot)
        X60, y60_reg, y60_class = prepare_data(df60, bot)

        # 3. Train Base Models (all as regressors)
        ...
        # 4. Train Meta-Model (as a classifier)
        print("\n--- Training Meta-Model ---")
