...
class DerivWS:
    def __init__(self, url, token):
        self.url      = url
        self.token    = token
        self._ws      = None          # underlying WebSocketApp
    ...
    def _on_ping(self, ws, data):
        # answer every ping immediately – keeps Deriv happy
    ...
    async def connect(self):
        ...
        while self._running:
            try:
                ...
                # run_forever() with ping_interval sent from client
                ...
                await asyncio.sleep(3)          # let socket open
                await self.authorize()
                return                          # success
            except Exception as e:
                log.error(f"connect fail: {e}  – retry in 10 s")
                await asyncio.sleep(10)

    async def authorize(self):
        await self.send({"authorize": self.token})
        await self.wait_for("authorize")
    ...
    async def wait_for(self, msg_type, timeout=15):
        ...
        while time.time() - t0 < timeout:
            try:
                ...
                continue
        raise TimeoutError(f"Waiting for {msg_type} timed out")
    ...
    async def get_candles(self, symbol, interval, count, style="candles"):
        ...
        return await self.wait_for("candles")
