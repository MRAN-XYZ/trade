...
class DerivTradingBot:
    ...
    def generate_rf_predictions(self, features):
        ...
        if len(features) > 100:
            ...
            # Check if we have all 3 classes in training data
            ...
            if len(proba) == 2:
                ...
                if 0 in self.rf_model.classes_ and 1 in self.rf_model.classes_:
                    # Has BUY (0) and SELL (1), missing HOLD (2)
            ...
            # Single class or other edge case
                return np.array([0.33, 0.33, 0.34])
    ...
    def generate_trading_signal(self):
        ...
        for timeframe in ['15min', '60min']:
            ...
            print(f"Feature columns for {timeframe}:", [type(col).__name__ for col in features.columns])
    ...
    async def connect_and_trade(self):
        """Run trading logic only once instead of looping"""
        try:
            async with websockets.connect(self.ws_url) as websocket:
                ...
                # Subscribe to JD10 ticks (just one tick)
                subscribe_request = {"ticks": "JD10", "subscribe": 1}
                await websocket.send(json.dumps(subscribe_request))

                # Subscribe to candles (15min + 60min)
                ...
                # --- Only receive one message, then stop ---
                response = await websocket.recv()
                data = json.loads(response)
                print(f"First WebSocket message: {data}")

                # If you want, process a single trading decision right away
                if 'tick' in data:
